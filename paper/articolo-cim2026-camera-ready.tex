...
\abstract
The live electronics of \brano, an ecosystemic feedback system immersed in its environment, drives its filter parameters from descriptors of the captured sound. Its original five-descriptor mapping was weak, because an absolute descriptor value moves with the room rather than with the gesture. My question: how to use a closed set of sixteen descriptors live, when the signal arrives through an environment and a chain. I gather evidence over three tests, each closer to the live signal. On synthetic signals descriptors are scale invariant yet drift under recording; on acoustic instruments the descriptor that follows the gesture depends on the instrument; and in an electroacoustics laboratory, with eight microphones over eight metres, the chain authors the measurement (air absorption, a standing wave, directivity, attenuation). The first three tests are the demonstrated part of this work; what follows is exploratory. My proposal is a relative, bipolar control between reference nodes: taken in the same environment as the input, the room's signature is common to both terms, and the comparison reads the gesture, not the room. Probed on signals recorded to study the chain, not to validate it, the result indicates the mechanism: the relative reading holds where the chain is stable and scrambles where the standing wave dominates. A dedicated real-time test remains.
\endabstract
...
è immerso nell'ambiente, in un anello dove ambiente, strumento e processo si co-determinano. Ogni parametro dei filtri è guidato da alcuni descrittori audio calcolati in tempo reale sul segnale in ingresso. All'inizio i parametri erano pilotati da cinque descrittori soltanto, uno per parametro, ma legarne ciascuno a un solo descrittore dava risultati poco differenziati, e cinque misure non bastavano a render conto di ciò che accadeva; così ho cominciato a studiarne il comportamento sui segnali sintetici, per poi estendere l'insieme a sedici.
...
L'analisi procede frame per frame con una FFT a 8192 campioni a 96 kHz (Hann, sovrapposizione del cinquanta per cento, risoluzione di circa 11,7 Hz per bin), sull'intera banda fino a Nyquist (48 kHz).%analizzo a banda piena di proposito, perché, come mostrerà l'esperimento di distanza, un tetto più basso nasconderebbe i fenomeni che vivono negli acuti.
 Per usare i descrittori fuori dal contesto ideale di ripresa è decisivo un gate a due livelli: uno relativo, componente per componente, che dentro ogni frame trattiene solo ciò che resta entro 60 dB dal picco; e uno che scarta gli interi frame troppo deboli (sotto una soglia assoluta o più di 30 dB sotto il picco del file), così da lasciar fuori il solo rumore di fondo ai bordi. Una normalizzazione z-score "congelata", con media e deviazione calcolate una volta sull'intero corpus e poi tenute fisse, rende i valori confrontabili fra sessioni diverse. L'intera pipeline è in Python e deterministica, e ne esiste una versione per il tempo reale come external in C verificata contro di essa; tutto il materiale (gli external Pure Data, i risultati in formato CSV e i grafici) è consultabile nel repository pubblico~\cite{reposupp}.
...
\bottomrule
\end{tabular*}
\caption{I sedici descrittori e le loro formule. $X(k)$ è l'ampiezza della $k$-esima bin della FFT, $N$ il numero di bin attive del frame, $E_i$ l'energia della $i$-esima banda ottavale e \mbox{$p(k)=|X(k)|^2/\sum_j|X(j)|^2$} l'energia normalizzata della bin $k$.}
...
\begin{figure}[t]
 ...
 \caption{Apparato della terza prova. A sinistra gli otto microfoni raggruppati a grappolo, stretti per ridurre la distanza fra le capsule mentre se ne calibrano i guadagni; a destra gli stessi microfoni distesi in fila a passi di un metro, da uno a otto metri dalla sorgente.}
 \label{fig:apparato}
\end{figure}

Prima dei singoli fenomeni, la Figura~\ref{fig:leap_corpus} dà il quadro d'insieme dei contrabbassi analizzati: si osserva immediatamente quanto il centroide si sposti da un punto di ripresa all'altro.

\begin{figure*}
 \centerline{\includegraphics[width=0.85\textwidth]{figura_leap_corpus.pdf}}
 \caption{Il contrabbasso nel terzo esperimento: per ogni campione (colonna: le due campane, originale e custom, nei due orientamenti del microfono) il centroide medio a banda piena agli otto microfoni; le linee uniscono lo stesso microfono fra i campioni (numero e colore del punto: vedi legenda).}
...
\begin{figure}[tbp]
 \centerline{\includegraphics[width=0.74\columnwidth]{figura_leap_onda_ccb.pdf}}
 \caption{Onda stazionaria della fondamentale del clarinetto contrabbasso nella stanza:
          ...
          La cresta al microfono~3 è un nodo (fondamentale debole, centroide alto),
          l'avvallamento ai microfoni 5--6 un ventre (fondamentale forte, centroide basso);
          la quasi coincidenza delle due curve esclude la direttività del microfono.}
 \label{fig:leap_onda}
\end{figure}
...
Un secondo segno lo lascia la campana sul Do tenuto. La kurtosis, indifferente all'onda sui cluster, sul Do sale da circa 21 al nodo a quasi 60 al ventre quando la campana custom è puntata di fronte alla capsula, mentre con la campana originale, o con la stessa campana e capsula ruotata, resta fra 10 e 13. Occorre il concorso di tre condizioni (la nota singola, il ventre dove la fondamentale è forte, la campana custom): la kurtosis non indica qui la distanza, ma l'interazione fra campana, luogo e microfono.
...
Quel punto merita attenzione, perché è il luogo in cui si vede la condizione di validità del metodo. Il bilanciamento richiede che la firma del luogo gravi in egual misura sul segnale e sui due nodi. Al ventre dell'onda la fondamentale è forte, e la Tabella~\ref{tab:onda} mostra che dodici descrittori su sedici rispondono proprio alla sua forza, sette in fase e quattro in opposizione. Su un asse largo, fra registri diversi, i due nodi stanno da parti opposte rispetto a quella leva e lo spostamento resta comune ai due termini del rapporto. Su un asse stretto, fra suoni dello stesso strumento, i due nodi condividono la fondamentale: l'onda li muove quasi insieme, la differenza fra le due distanze si assottiglia, e con essa l'informazione che $v$ trasporta. Non è la distanza fra i nodi a determinare la lettura, ma la coincidenza fra ciò su cui poggia l'asse e ciò su cui agisce il luogo. Per l'uso dal vivo ne segue che la tenuta di un asse stretto dipende anche da dove il microfono cade rispetto all'onda della fondamentale in gioco, un fattore che resta da caratterizzare (Sezione~\ref{sec:discussione}).

Va però chiarito con quali suoni lo sto provando: registrati per studiare la catena, non per saggiare i nodi, questi numeri indicano il meccanismo più che una stima conclusiva del controllo. Un cluster, poi, non è un punto fermo nello spazio dei descrittori: al suo interno il suono si muove di continuo, e trattarlo come riferimento unico è una semplificazione.

Conta semmai quali suoni si scelgono come nodi: un polo deve restare distintivo anche sotto ripresa. Un suono indifferenziato come il rumore bianco è un cattivo polo, perché passando per la catena, che ne taglia gli acuti, perde la sua collocazione estrema e si sposta verso il centro della distribuzione, e l'asse degenera (quasi ogni segnale finisce a ridosso di un polo). I suoni reali, che restano riconoscibili sotto microfono e vivono già nello stesso ambiente dell'ingresso, non hanno questo difetto, ed è con loro che vale la tenuta della Figura~\ref{fig:controllonodi}.

Questa tenuta vale però per la distanza euclidea sugli z-score, non per lo sbiancamento di Mahalanobis~\cite{mahalanobis1936generalised}, ed è qui che il suo ruolo emerge con chiarezza: lo sbiancamento è piuttosto uno strumento diagnostico da usare in fase di taratura, non una correzione da applicare sempre. Amplificando flux e irregularity, i descrittori di varianza ridotta nel corpus, dà peso proprio all'irregolarità che l'ambiente aggiunge, e sotto ripresa peggiora la tenuta del controllo invece di migliorarla.

Una scelta va dichiarata: il controllo collassa i sedici descrittori in una sola distanza, e a runtime l'interpretabilità, quale descrittore si è mosso e perché, non si legge più. È voluto. L'interpretabilità serve a monte, nell'analisi e nella taratura (scegliere i nodi, capire con lo sbiancamento se un asse poggia sulla sola brillantezza); il controllo dal vivo è deliberatamente sintetico, un numero solo lungo l'asse scelto. E i sedici entrano tutti nella distanza, compresi i più fragili sotto catena (crest, skewness, kurtosis, numero di picchi): non perché siano affidabili in assoluto, ma perché quali descrittori portino un dato asse non è deducibile a priori (lo mostra l'onda nodo/ventre). Una pesatura per-asse, che attenui i descrittori che non variano su quell'asse, è il raffinamento naturale.

L'idea si estende a più dimensioni: più coppie di nodi, ciascuna lungo un proprio asse fra un polo $+1$ e un polo $-1$, generano più valori $v$ simultanei dallo stesso ingresso, cioè una superficie di controllo dove ogni asse corrisponde a una direzione musicale scelta in calibrazione o ridefinita lungo il brano. È a questo controllo che in \brano affido la guida del comportamento elettronico dal vivo. Nell'anello ecosistemico questo ha un senso preciso: il sistema legge la propria posizione rispetto a riferimenti che vivono nello stesso ambiente e si muovono con esso. L'acustica del luogo diventa parte condivisa del confronto.

I nodi possono infine cambiare dal vivo: il sistema può promuovere a nodo il segnale ripreso in un istante, la media degli ultimi secondi suonati, o un suono sintetico verso cui tendere. Cambiare un nodo ribalta di colpo tutti i rapporti, perché ciò che fino a un attimo prima leggeva in un modo ora si misura rispetto a un altro riferimento: un'articolazione strutturale del brano.

Anche la distanza fra i nodi nello spazio dei descrittori è una scelta di progetto: allontanarli rende la lettura robusta ma grossolana, avvicinarli la rende fine ma fragile, fino a due nodi coincidenti dove l'asse si annulla. Un criterio sistematico per tararla resta fra i lavori futuri.
